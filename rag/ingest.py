...
Walks knowledge_base/{crimes,procedures}/*.{txt,md,pdf,docx}, extracts text,
chunks it, embeds each chunk, and upserts into Pinecone's default namespace
with metadata (source file, category, chunk text) so the retriever can cite
it. The "reports" namespace (duplicate detection, see
ai/duplicate_detector.py) is populated at submission time, not by this script.
"""
...
def iter_documents():
    for category_dir in KB_DIR.iterdir():
        if not category_dir.is_dir():
            continue
        category = category_dir.name  # "crimes" / "procedures"
